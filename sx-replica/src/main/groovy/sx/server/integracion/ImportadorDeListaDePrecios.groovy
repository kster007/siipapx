...
import sx.compras.ListaDePreciosDet
...
class ImportadorDeListaDePrecios implements Importador {
    ...
    def importar(Date ini, Date fin){
        logger.info("Importando listad de precios del : ${ini.format('dd/MM/yyyy')} al ${ini.format('dd/MM/yyyy')}" )
    ...
    def build(def row){
        def lista = ListaDePrecios.where{ sw2 == row.sw2}.find()
        if(!lista){
            lista = new ListaDePrecios()
    ...
    def importarPartidas(ListaDePrecios lista){
    ...
    static String QUERY_PARTIDAS ="""
        select
            ...
            l.desc5 as descuento5,
            l.desc6 as descuento6,
